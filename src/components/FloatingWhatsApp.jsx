import { motion } from 'framer-motion'

// Bouton WhatsApp flottant - même esprit que le site Elie (vert #25D366,
// entrée animée, hover qui grossit), adapté aux conventions de ce projet.
export default function FloatingWhatsApp() {
  return (
    <>
      <motion.a
        href="https://wa.me/33634084690?text=Bonjour%20Renaud%2C%20j'aimerais%20parler%20d'un%20projet."
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Contacter Renaud sur WhatsApp"
        className="wa-fab"
        initial={{ scale: 0, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 1, type: 'spring', stiffness: 200, damping: 16 }}
        whileHover={{ scale: 1.08 }}
        whileTap={{ scale: 0.94 }}
      >
        <svg width="30" height="30" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
          <path d="M19.05 4.91A9.82 9.82 0 0 0 12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.86 9.86 0 0 0 4.79 1.22h.004c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.02zM12.04 20.13h-.003a8.2 8.2 0 0 1-4.18-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.18 8.18 0 0 1-1.26-4.36c0-4.54 3.7-8.23 8.24-8.23a8.2 8.2 0 0 1 8.23 8.24c0 4.54-3.7 8.23-8.24 8.23zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.25-.64.81-.79.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.01-.38.11-.5.11-.11.25-.29.37-.43.13-.14.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.41-.42-.56-.43l-.48-.01c-.17 0-.43.06-.66.31-.22.25-.86.85-.86 2.07 0 1.22.89 2.4 1.01 2.56.12.17 1.75 2.67 4.23 3.74.59.26 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.07.14-1.18-.06-.1-.22-.16-.47-.28z" />
        </svg>
      </motion.a>

      <style>{`
        .wa-fab {
          position: fixed; bottom: 24px; right: 24px; z-index: 950;
          display: inline-flex; align-items: center; justify-content: center;
          width: 56px; height: 56px; border-radius: 50%;
          background: #25D366; color: #fff;
          box-shadow: 0 6px 20px rgba(37,211,102,0.45), 0 2px 10px rgba(20,15,9,0.22);
          transition: box-shadow var(--dur-mid) var(--ease);
        }
        .wa-fab:hover {
          box-shadow: 0 10px 28px rgba(37,211,102,0.55), 0 4px 14px rgba(20,15,9,0.26);
        }
        .wa-fab:focus-visible { outline: 2px solid #fff; outline-offset: 3px; }
        /* sur mobile, on remonte au-dessus de la barre FloatingCTA */
        @media (max-width: 760px) {
          .wa-fab { bottom: 88px; right: 16px; width: 52px; height: 52px; }
        }
      `}</style>
    </>
  )
}
